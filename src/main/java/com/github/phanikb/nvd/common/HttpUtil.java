...
public final class HttpUtil {
    ...
    public static TimeValue getDownloadDelay() {
        ...
        int windowSizeInSecs = properties.getNvd().getDownload().getUsingApi().getProcessor().getRollingWindowInSecs();

        if (rateLimit > 0) {
            long minDelay = (windowSizeInSecs * 1000L) / rateLimit;
            return TimeValue.ofMilliseconds(Stream.of(configuredDelay,
                                                      minDelay,
                                                      DEFAULT_DELAY_BETWEEN_REQUESTS_IN_MS).max(Long::compare)
    ...
    public static int getRateLimit() {
        int rateLimitWithKey = properties.getNvd().getDownload().getUsingApi().getProcessor().getRateLimitWithKey();
        int rateLimitWithoutKey = properties.getNvd().getDownload().getUsingApi().getProcessor().getRateLimitWithoutKey();
    ...
    public static <T> T getApiJson(URI uri, CloseableHttpClient httpclient, HttpClientResponseHandler<T> responseHandler) throws IOException, NvdException {
    ...
    public static <T> void downloadHttpGetRequest(URI uri, File outFile, HttpClientResponseHandler<T> responseHandler) throws NvdException {
        // replaceUriHeader("User-Agent", getUserAgent());
        try (CloseableHttpClient httpclient = HttpClients.custom().setRetryStrategy(
                new DefaultHttpRequestRetryStrategy(HttpUtil.getMaxRetries(),
                                                    TimeValue.ofSeconds(
                                                            HttpUtil.getRetryIntervalInSecs()))).setDefaultHeaders(
                HttpUtil.getNvdDefaultHeaders()).build()) {
