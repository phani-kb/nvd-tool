...
public class CpeApiOptions extends LastModApiOptions {
    ...
    @CommandLine.Option(names = {"--cpe-name-id"}, paramLabel = "ID",
            description = "Returns a specific CPE record identified by a Universal Unique Identifier (uuid).")
    private UUID cpeNameId;

    @CommandLine.Option(names = {"--cpe-ms"}, paramLabel = "STRING", converter = CpeName.class,
            description = "Returns CPE Names that exist in the Official CPE Dictionary, based on the value of {match string}.")
    ...
    public static class KeywordSearch {
        @CommandLine.Option(names = {"--kw-em"}, description = "Returns only the CPE matching the phrase exactly.")
        private boolean keywordExactMatch;

        @CommandLine.Option(names = {"--kw-search"}, paramLabel = "STRING",
                description = "Returns any CPE record where a word or phrase is found in the metadata title or reference links.")
